...
每个切换窗口必须完成：冻结旧入口、导入最终增量、对账、启用 Registry writer fence、撤销 Legacy 数据库直写权限、验证 API/事件幂等、记录切换版本和负责人。
...
Archive synthetic 兼容修正在共享工作区中已经实现，但尚未提交或部署；真实图片、生产 ID 清单、快照存储、路由和 cutover 审查仍全部开放。
